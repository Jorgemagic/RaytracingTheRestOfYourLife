...
namespace _21_CornellBoxGlassSphere
{
    public class Helpers
    {
        ...
        public Vector3 Random_to_sphere(float radius, float distance_squared)
        {
            float r1 = (float)random.NextDouble();
            float r2 = (float)random.NextDouble();
            float z = 1 + r2 * ((float)Math.Sqrt(1 - radius * radius / distance_squared) - 1);

            float phi = 2 * (float)Math.PI * r1;
            float x = (float)Math.Cos(phi) * (float)Math.Sqrt(1 - z * z);
            float y = (float)Math.Sin(phi) * (float)Math.Sqrt(1 - z * z);
