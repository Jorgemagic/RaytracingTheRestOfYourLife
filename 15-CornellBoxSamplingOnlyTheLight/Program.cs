...
namespace _15_CornellBoxSamplingOnlyTheLight
{
    public class Program
    {
        ...
        static Vector3 Ray_color(Ray r, Vector3 background, HitTable world, int depth)
        {
            ...
            if (!rec.Mat_ptr.Scatter(r, rec, out albedo, out scattered, out pdf))
                return emitted;

            var on_light = new Vector3(Helpers.RandomFloat(213, 343), 554, Helpers.RandomFloat(227, 332));
            var to_light = on_light - rec.P;
            var distance_squared = to_light.LengthSquared();
            to_light = Vector3.Normalize(to_light);

            if (Vector3.Dot(to_light, rec.Normal) < 0)
                return emitted;

            float light_area = (343 - 213) * (332 - 227);
            float light_cosine = Math.Abs(to_light.Y);
            if (light_cosine < 0.000001f)
                return emitted;

            pdf = distance_squared / (light_cosine * light_area);
            scattered = new Ray(rec.P, to_light, r.Time);
